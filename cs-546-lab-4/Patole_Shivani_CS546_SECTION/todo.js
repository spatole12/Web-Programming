...
module.exports = {
    ...
    async createTask(title1, description1) {
        ...
        let todoCollection = await todoColl().catch(error => {
            console.log(error);
        });
        let insert_task = await todoCollection.insertOne(input_obj).catch(error => {
            console.log(error);
        });
    ...
    async getAllTasks() {
        const todoCollection = await todoColl().catch(error => {
            console.log(error);
        });
        const todo = await todoCollection.find({}).toArray().catch(error => {
            console.log(error);
        });
    ...
    async getTask(id) {
        ...
        const todo_get = await todoCollection.findOne({ _id: id }).catch(error => {
            console.log(error);
        });
    ...
    async completeTask(taskId) {
        ...
        const updated_task = await todoCollection.updateMany({ _id: taskId }, { $set: { completed: "true", completedAt: new Date() } }).catch(error => {
            console.log(error);
        });
    ...
    async removeTask(id) {
        ...
        const deletionInfo = await todoCollection.removeOne({ _id: id }).catch(error => {
            console.log(error);
        });
